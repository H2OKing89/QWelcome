...
class ImportExportRepositoryTest {
    ...
    @Test
    fun `validateImport returns invalid when payload exceeds hard limit by length`() =
        runTest {
    ...
    @Test
    fun `validateImport returns invalid when utf8 byte size exceeds hard limit`() =
        runTest {
    ...
    @Test
    fun `exportTemplatePack returns localized no templates message when nothing to export`() =
        runTest {
    ...
    @Test
    fun `exportTemplatePack returns specific message when built in default is explicitly requested`() =
        runTest {
            coEvery { settingsStore.getAllTemplates() } returns
                listOf(
                    Template(
    ...
    @Test
    fun `validateImport returns invalid when full backup has empty templates`() =
        runTest {
            val backup =
                FullBackup.create(
    ...
    @Test
    fun `legacy full backup with loadout keys validates and applies`() =
        runTest {
    ...
    @Test
    fun `full backup restore includes selected tech profile in atomic write`() =
        runTest {
    ...
    @Test
    fun `full backup restore leaves optional profile and active template unchanged`() =
        runTest {
            val backup =
                FullBackup.create(
